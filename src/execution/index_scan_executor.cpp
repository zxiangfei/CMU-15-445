...
//  * @LastEditTime: 2025-08-25 15:37:43
 ...
 * @Date: 2025-07-02 22:24:46
 * @LastEditors: zxiangfei 2464257291@qq.com
 * @LastEditTime: 2025-09-01 00:10:00
 * @FilePath: /CMU-15-445/src/execution/index_scan_executor.cpp
 */
...
void IndexScanExecutor::Init() {
  ...
  // 表堆与索引
  table_heap_ = GetExecutorContext()->GetCatalog()->GetTable(plan_->table_oid_)->table_.get();
  auto index_info = GetExecutorContext()->GetCatalog()->GetIndex(plan_->index_oid_);
  htable_ = dynamic_cast<BPlusTreeIndexForTwoIntegerColumn *>(index_info->index_.get());

  // Fall 2024：主键整型索引，支持点查与有序遍历
  if (!plan_->pred_keys_.empty()) {
    for (const auto &pred_key : plan_->pred_keys_) {
      Value key_value =
          pred_key->Evaluate(nullptr, GetExecutorContext()->GetCatalog()->GetTable(plan_->table_oid_)->schema_);
      Tuple index_key({key_value}, htable_->GetKeySchema());
      std::vector<RID> rids;
      htable_->ScanKey(index_key, &rids, GetExecutorContext()->GetTransaction());
      result_rids_.insert(result_rids_.end(), rids.begin(), rids.end());
    }
    current_index_ = 0;
  } else if (plan_->filter_predicate_ != nullptr) {
    // 单个可下推的等值过滤 → 点查
    auto value = plan_->filter_predicate_->Evaluate(
        nullptr, GetExecutorContext()->GetCatalog()->GetTable(plan_->table_oid_)->schema_);
    Tuple index_key({value}, htable_->GetKeySchema());
    std::vector<RID> rids;
    htable_->ScanKey(index_key, &rids, GetExecutorContext()->GetTransaction());
    result_rids_.insert(result_rids_.end(), rids.begin(), rids.end());
    current_index_ = 0;
    ...
    // 全索引遍历
    iterator_ = std::make_unique<BPlusTreeIndexIteratorForTwoIntegerColumn>(htable_->GetBeginIterator());
  }
}

// 尝试将一个 RID 在当前快照下重建为可见版本。可见则写入 out_tuple 并返回 true，否则返回 false。
static inline auto ReconstructVisibleTuple(TransactionManager *txn_mgr, TableHeap *table_heap,
                                           const Schema &table_schema, Transaction *txn, const RID &rid,
                                           Tuple *out_tuple) -> bool {
  // 取表堆头部 + 最新 undo_link
  TupleMeta base_meta;
  Tuple base_tuple;
  std::optional<UndoLink> link;
  std::tie(base_meta, base_tuple, link) = GetTupleAndUndoLink(txn_mgr, table_heap, rid);

  // 基于 read_ts 收集“刚好够用”的 undo 序列：
  //   - 空向量：头版本对我可见
  //   - nullopt：这个 RID 在我快照里不存在（比如被更早的删除）
  auto undo_logs_opt = CollectUndoLogs(rid, base_meta, base_tuple, link, txn, txn_mgr);
  if (!undo_logs_opt.has_value()) {
    return false;
  }

  // 应用这段 undo，重建出“对当前事务可见”的版本
  auto reconstructed = ReconstructTuple(&table_schema, base_tuple, base_meta, *undo_logs_opt);
  if (!reconstructed.has_value()) {
    return false;  // 对当前事务来说，这个 RID 被视为“已删除”
  }

  *out_tuple = std::move(reconstructed.value());
  out_tuple->SetRid(rid);
  return true;
}
...
auto IndexScanExecutor::Next(Tuple *tuple, RID *rid) -> bool {
  ...
  const Schema &table_schema = GetExecutorContext()->GetCatalog()->GetTable(plan_->table_oid_)->schema_;

  // 点查路径
  if (!plan_->pred_keys_.empty() || plan_->filter_predicate_ != nullptr) {
    while (current_index_ < result_rids_.size()) {
      const RID cand = result_rids_[current_index_++];

      Tuple visible;
      if (ReconstructVisibleTuple(txn_mgr, table_heap_, table_schema, txn, cand, &visible)) {
        *rid = cand;
        *tuple = std::move(visible);
        return true;
      }
    }
    return false;
  ...
  while (!iterator_->IsEnd()) {
    auto [key, value] = **iterator_;
    ++(*iterator_);

    Tuple visible;
    if (ReconstructVisibleTuple(txn_mgr, table_heap_, table_schema, txn, value, &visible)) {
      *rid = value;
      *tuple = std::move(visible);
      return true;
    }
