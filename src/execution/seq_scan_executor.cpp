 ...
 * @LastEditTime: 2025-09-02 11:21:56
  ...
  while (current_rid_ != rids_.end()) {
    ...
    if (out_tuple.has_value()) {
      ...
      if (plan_->filter_predicate_) {
        auto val = plan_->filter_predicate_
                       ->Evaluate(&out_tuple.value(),
                                  GetExecutorContext()->GetCatalog()->GetTable(plan_->GetTableOid())->schema_)
                       .GetAs<bool>();
        if (!val) {
